#define NB_CLIENT_TRIOS 1 // 
// Si NB_STATION > NB_CLIENT_TRIOS*3, alors le checker ne termine jamais
#define NB_STATION 4
#define client_id_t int
#define station_id_t int
#define TAILLE_FILE_DATTENTE (NB_CLIENT_TRIOS*3)

// Quel type de message peuvent etre envoyer.
mtype = { DIESEL, ESSENCE, ELEC };

// Le channel "lineup" represente les clients qui attendent pour un guichet
chan lineup = [NB_CLIENT_TRIOS*3] of {client_id_t};

// Il y a un channel par client, afin que le guichet puisse notifier chaque client individuellement.
chan client_chan[NB_CLIENT_TRIOS*3] = [0] of {client_id_t};
chan client_station[NB_CLIENT_TRIOS*3] = [0] of {station_id_t, mtype};
...
proctype client(client_id_t id; mtype engine_type)
{
    ...
    do
        ::  printf("Client %d: J'entre dans la station-service\n", id);
            ...
            // Attendre qu'une station delivre la commande
...
init {
    ...
    // Ne pas oublier de mettre NB_STATION a 2
    //ltl q2_2 { []<>client@client_in_line -> []<>client@client_order_placed }
}
